...
from numpy import (array, unravel_index, nditer, linalg, random, subtract, max,
                   ...
                   einsum, prod, nan, sqrt, hstack, diff, argmin, multiply,size,shape, ndim)
...
class MiniSom(object):
    def __init__(self, x, y, input_len, sigma=1.0, learning_rate=0.5,
                 ...
                 activation_distance='euclidean', random_seed=None, som_method = 'sequential'):
        ...
        method_functions = {'sequential': self.train,
    ...
    def winner(self, x):
        ...
        """ Author: steve hong update 4/7/2022  """
#          if ndim(x) == 1:
        self._activate(x)
...
#             return winner_chunk
    ...
    def update(self, input_data , t, max_iteration):
        ...
        input_data : np.array
            the input data, use all_data to 
        win_list : list
            List of positions of the winning neurons for x (array or tuple).
        t : int
            Iteration index
        max_iteration : int
        ...
        """ Author: steve hong update 8/7/2022  """
...
#         numerator = 0 
        denominator = 0
        new_weights = zeros(shape(self._weights))
        
        #We loop over observations and calculate numerator and denominator
        #Numerator is stored as new_weights and has the correct shape 
        #Denominator is the same for all the 7 features (specific to seed data)    
        for i in arange(size(input_data,0)):
#             win = win_list[i]
            win = self.winner(input_data[i])
            g = self.neighborhood(win, sig)*eta
            denominator += g
            for j in arange(size(input_data,1)):
#                 numerator += g * data[i]
                new_weights[:,:,j] += g * input_data[i,j]
        
        #We need the results to be in (9,9,7) form
        #Divide by the denominator
        for j in arange(size(input_data,1)):
            new_weights[:,:,j] = new_weights[:,:,j] / denominator
        self._weights = new_weights
          
                
#                 h_list = np.exp(-np.power(pairwise_distances([[i,j]],win),2))/(2*(sig**2))
#                 h_list = np.reshape(h_list, (len(input_data),))
#                 denominator = np.sum(h_list)
                
#                 #Initiate [0,0,0,0,0,0,0] then add h*data one by one
#                 numerator = [0] * len(np.transpose(input_data))
#                 for index in range(len(h_list)):
#                     numerator += h_list[index] * input_data[index]
                    
                
#                 self._weights[i,j] = numerator/denominator      
#         return self._weights
    ...
    def train(self, data, num_iteration, random_order=False, verbose=False):
        ...
        for t, iteration in enumerate(iterations):
            #Batch Update
            self.update(data, t, num_iteration)
        if verbose:
            print('\n quantization error:', self.quantization_error(data))

    def trainBatch(self, data, num_iteration, random_order=False, verbose=False):
        """Trains the SOM.
        ...
        print( "\nITERATIONS" )
        for t, iteration in enumerate(iterations):
            print( "t=", t, "; Iteration value=", iteration, "; winner=", self.winner(data[iteration]), "; data iteration = ", data[iteration] )
